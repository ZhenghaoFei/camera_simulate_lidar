...
import simladar_input
FLAGS = tf.app.flags.FLAGS

# Basic model parameters.
tf.app.flags.DEFINE_integer('batch_size', 5,
                            """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('data_dir', '../data/data1/',
...
def inputs(eval_data=False):
  ...
  left_batch, right_batch, lidar_batch = simladar_input.inputs(eval_data=eval_data,
                                        data_dir=data_dir,
                                        batch_size=FLAGS.batch_size)
  if FLAGS.use_fp16:
    left_batch = tf.cast(left_batch, tf.float16)
    right_batch = tf.cast(right_batch, tf.float16)
    lidar_batch = tf.cast(lidar_batch, tf.float16)
...
def inference(left_images, right_images):
  ...
  # norm2
  left_norm2 = tf.nn.lrn(left_conv2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                    name='left_norm2')
  # right image
    # right_conv1
  ...
  # norm1
  right_norm1 = tf.nn.lrn(right_conv1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                    name='right_norm1')
  ...
  # norm2
  right_norm2 = tf.nn.lrn(right_conv2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75,
                    name='right_norm2')
  ...
  with tf.variable_scope('FC1') as scope:
    ...
    weights = _variable_with_weight_decay('weights', [dim, 120],
                                          stddev=1/dim, wd=0.0)
